...
export class SignUpDialog {
	...
	sendAuthenticationMail() {
		...
		this.svcApi.authEmail({
			"email": email
		}).subscribe({
			next: (res) => {
				this.checkAuthenticationCode = res.message;
				console.log(this.checkAuthenticationCode);
	...
	authenticationVerification() {
		const authCode = this.signUpForm.get('authenticationCode')?.value;
		console.log(authCode);
		if (!authCode) return;
		if (authCode === this.checkAuthenticationCode) {
			this.emailAuthentication = false;
			this.authenticationSuccessful = true;
		} else {
			return window.alert("인증에 실패하셨습니다. 인증코드를 다시 확인해주세요.");
		}
	}
	...
	getEmailErrorMessage() {
		...
		if (email?.hasError('email')) return "* 이메일 형식에 맞게 입력해주세요.";
